...
Examples:
  $(basename "$0") --manifest manifest.json --tape-host admin@tape.example.com --tape-path /mnt/tape/archive
  $(basename "$0") --manifest manifest.json --tape-host admin@tape.example.com --tape-path /mnt/tape/archive --tar archive_001.tar
  $(basename "$0") --manifest manifest.json --tape-host admin@tape.example.com --tape-path /mnt/tape/archive --checksum
EOF
    exit "${1:-0}"
}
